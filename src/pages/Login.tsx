...
const Login: React.FC<LoginProps> = () => {
  ...
  const processTwitchOAuthCallback = useCallback(async (code: string, state: string) => {
    try {
      ...
      const widget = `${baseUrl}/?twitchChannel=${userData.username}&twitchToken=${tokenResponse.access_token}`;
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (twitchToken && twitchUser) {
        ...
        let widget = `${baseUrl}/?twitchChannel=${userData.username}&twitchToken=${twitchToken}`;
        ...
        setKickChannel(savedKickChannel);
        setKickWidgetUrl(`${baseUrl}/?kickChannel=${encodeURIComponent(savedKickChannel)}`);
      }
    };
  ...
  const handleKickChannelSubmit = () => {
    ...
    if (twitchWidgetUrl) {
      ...
      const url = `${baseUrl}/?kickChannel=${encodeURIComponent(trimmedChannel)}`;
